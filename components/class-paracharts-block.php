...
class Paracharts_Block {
	/**
	 * Constructor
	 */
	public function __construct() {
		add_action( 'init', [ $this, 'register_paracharts_block_support' ] );
		add_action( 'plugins_loaded', [ $this, 'wt_paracharts_load_textdomain' ] );
		add_action( 'rest_api_init', [ $this, 'register_fetch_paracharts_options' ] );
	...
	public function register_paracharts_block_support() {
		...
		wp_register_script(
			'paracharts-editor',
			paracharts()->plugin_url . '/components/block/index.js',
			[ 'wp-i18n' ],
			$this->version_str(),
			true
		);
		...
		wp_register_style(
			'paracharts-style',
			paracharts()->plugin_url . '/components/block/index.css',
			[],
			$this->version_str()
		);
	...
	public function version_str() {
		$plugin_file    = plugin_dir_path( __DIR__ ) . ( 'paracharts.php' );
	 ...
	 */
	public function wt_paracharts_load_textdomain() {
		load_plugin_textdomain( 'paracharts', false, basename( __DIR__ ) . '/languages' );
	}
	 ...
	 */
	public function register_fetch_paracharts_options() {
		register_rest_route(
			'paracharts/v1',
			'/options/',
			[
	...
	public function fetch_options() {
		$paracharts_options = get_option( 'paracharts' );
		$image_support  = $paracharts_options && is_array( $paracharts_options ) ? $paracharts_options['performance'] === 'default' : true;
		$args           = [ 'post_type' => 'paracharts', 'post_status' => 'publish' ];
	...
	public function fetch_graphs( $request ) {
		...
		foreach ( $posts as $post ) {
			$result             = [];
			$post_meta          = get_post_meta( $post->ID, 'paracharts', true );
