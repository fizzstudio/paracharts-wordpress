...
export default function edit( { attributes, setAttributes } ) {
    ...
    // Url constants.
    const newUrl = `${ siteUrl }/wp-admin/post-new.php?post_type=paracharts`;
    const editUrl = `${ siteUrl }/wp-admin/post.php?post=${ attributes.chartId }&action=edit`;
    const optionsUrl = `/paracharts/v1/options`;

    // Blockprops.
    const blockProps = useBlockProps( { className: 'paracharts-block-chart-selector' } );
    // Miscellaneous
    ...
    const fetchGraphs = ( value ) => {
        ...
        apiFetch( { path: `/paracharts/v1/graphs/${ value }` } )
            .then(
                result => {
                    let charts = [];
    ...
    return (
        <div { ...blockProps }>
            <BlockControls>
                <ToolbarGroup className="paracharts-block">
                    <ToolbarButton onClick={ () => window.location.href = newUrl } icon="external">{ __( 'New Chart', 'paracharts' ) }</ToolbarButton>
                    { attributes.chartId &&
                        <>
                            <ToolbarButton onClick={ () => window.location.href = editUrl } icon="external" >{ __( 'Edit Chart', 'paracharts' ) }</ToolbarButton>
                            <ToolbarButton onClick={ () => handleClick( '' ) } >{ __( 'Replace', 'paracharts' ) }</ToolbarButton>
                        </>
                    }
                </ToolbarGroup>
            </BlockControls>

            <div className="wp-block paracharts-selector">
				<div className="components-placeholder block-editor-paracharts-placeholder is-large">
					
	                <div className="components-placeholder__label"><span className="dashicons dashicons-chart-pie"></span>Chart</div>
	                <div className="viewbox">
	                    { loadProblem ?
	                        <p>{ __( 'There was a problem fetching charts', 'paracharts' ) }</p>
	                        :
	                        <>
	                            { !loaded ?
	                                <p className="center">
	                                ...
	                                attributes.chartId ?
	                                    ...
	                                    :
	                                    maxAvailable === 0 ?
	                                        <p className="center">{ __( 'No Charts found', 'paracharts' ) }
	                                            <a href={ newUrl }>{ __( 'Create a new chart', 'paracharts' ) }</a>
	                                        </p>
	                                        :
	                                        <div className="no-chart-selected">
	                                            <div className="search-box">
	                                                <TextControl
	                                                    value={ search }
	                                                    placeholder={ __( 'Search by title', 'paracharts' ) }
	                                                    onChange={ ( value ) => handleSearch( value ) }
	                                                    autoFocus
	                                                />
	                                                <p className="count">{ optionsList.length } { __( ' of ', 'paracharts' ) }{ available }</p>
	                                            </div>
	                                            { optionsList.length === 0 && search.length > 1 ?
	                                                <p>{ __( 'No Charts found using this search string', 'paracharts' ) }</p>
	                                                :
